...
// Keep ser, sclk, rclk as they rarely change over time

var _ser=-1, _sclk=-1, _rclk=-1;

function setup(ser, sclk, rclk) {
  _ser=ser;
  _sclk=sclk;
  _rclk=rclk;
  _ser.writeSync(0);
...
// Note that RCLK is not triggered
...
function getBitMapFromChar(character) {
  if (character>=32 && character <= 128)
    return sixteenSegmentChars[character-32];
  return 0;
}
...
function displayChars() {
  ...
  return function() {
    data[0]=getBitMapFromChar(currentChar++);
    lowerRclk();
    sendBits(data, 1);
    raiseRclk();
    if (currentChar==128) currentChar=32;
  }
}
